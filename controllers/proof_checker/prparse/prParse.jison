...
defs
    :  defs proof  { $$ = $1; $$.push($2); }
    |  defs abbrev { $$ = $1; $$.push($2); }
    |  /* empty */ { $$ = []; }
    ;

abbrev
    // precedence decl to resolve conflict with transPred
    // This says "don't reduce def until you've gotten as much as you can"
    :  DEF SYMBOL '(' vardecllist ')' ':' e END
    {
       var sym = makeExpr('Symbol', [$2]);
       $$ = makeExpr('def', [sym, makeExpr('\\lambda', [$4, $7 ])]);
    }
    ;


/* FIXME: label optional for top-level subproof? */
proof
     : label PROOF varlist conclusions END {
            $$ = ['PROOF', $1];
	    $$.push($3);
	    $$ = $$.concat($4);
         }
...
varlist:  'VAR' vardecllist        { $$ = $2; }
    | /* empty */		   { $$ = makeExpr('vardecllist', []); }
    ;
...
conclusions
    :   conclusion { $$ = [$1]; }
    |   abbrev  { $$ = [$1]; }
    |   conclusions conclusion {$$ = $1; $$.push($2); }
    |   conclusions abbrev {$$ = $1; $$.push($2); }
    ;

conclusion
    :  label e 'BY' justification { $$ = ['CONCLUSION', $1, $2, $4]; }
    |  transPred e 'BY' justification {
    	  $$ = ['CONCLUSION', null, makeExpr($1, [exprProto.lhsMarker, $2]), $4];
	}
    |  proof  { $$ = $1; }
    |  error
    ;
...
justification
    :  SYMBOL 		{ $$ = [$1]; }
    |  SYMBOL '(' justargs ')' { $$ = $3; $$.unshift($1); }
    ;

justargs
    :  SYMBOL		{ $$ = [$1]; }
    |  justargs ',' SYMBOL  { $$ = $1; $$.push($3); }
    ;
