...
function SRFLAtoWorld(set) {
  // console.log(set);
	...
	for (var i = 0; i < set.length; i++) {
		...
		$(id).attr('src', img).droppable({accept: function() {return true;}, tolerance:'pointer', drop: handleDrop, acceptsSelf: true}).draggable({cursor: 'move', helper: 'clone', opacity: constants.DRAG_OPACITY, start: handleMoveStart, stop: handleMoveStop, drag: handleDrag}).css('cursor', 'move').addClass('COLOR_'+
